"""Run the standard utility evaluation."""
...
def run(
    ...
    if code != 0:
        return code
    if dry_run:
        report_evaluation_dry_run(ctx, "utility_df_full.csv", selected, copies)
        return 0
    evaluate_utility(ctx, selected, copies=copies)
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the standard utility evaluation.")
