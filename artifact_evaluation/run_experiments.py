...
This script is designed to run from the artifact_evaluation/ folder and use only files under it.

It supports:
- generation: PROVGEN, internal LDP, PrivBayes, and DPSyn
- evaluation: GWAS, MIA, and utility metrics computed from generated datasets
- validate-only mode to verify data/config wiring before long jobs
"""
...
DPSYN_DELTA = 3.4498908254380166e-11
...
class Context:
    ...
    @property
    def comparison_methods_dir(self) -> Path:
        return self.root / "comparison_methods"
...
def load_shared_data(ctx: Context, dataset: DATASET, epsilon: float, method: str, snp_count: int = 0, idx: int = 0) -> np.ndarray:
    ...
    if method == "dpsyn":
        df = clean_dataframe(pd.read_csv(path))
        drop_cols = [col for col in ["PATIENT_ID", "epsilon"] if col in df.columns]
        if drop_cols:
...
def baseline_output_is_valid(
    ...
    path = shared_path(ctx, dataset, epsilon, method, snp_count=snp_count, idx=idx)
    if not path.exists():
        return False

    try:
        original = load_target_data(ctx, dataset, snp_count=snp_count, idx=idx)
        shared = load_shared_data(ctx, dataset, epsilon, method, snp_count=snp_count, idx=idx)
    except Exception as exc:  # pragma: no cover - defensive validation for third-party outputs
        print(f"[warn] Existing {method} output at {path} could not be loaded ({exc}); regenerating")
        return False

    if shared.shape != original.shape:
        print(
            f"[warn] Existing {method} output at {path} has shape {shared.shape}, "
            f"expected {original.shape}; regenerating"
        )
        return False
    return True
...
def _integer_dtype_name(values: pd.Series) -> str:
...
def write_dpsyn_run_config(
    *,
    ...
    """Create DPSyn's per-run schema files from the exact cleansed CSV being synthesized."""
    df = pd.read_csv(priv_data)
    schema: dict[str, dict] = {}
    dtype_map: dict[str, str] = {}

    for column in df.columns:
        series = pd.to_numeric(df[column], errors="coerce").dropna().astype(int)
        dtype = _integer_dtype_name(series)
        ...
        values = sorted(int(value) for value in series.unique())
        schema[column] = {
            "dtype": dtype,
            "kind": "categorical",
            "values": values,
            "codes": list(range(1, len(values) + 1)),
        }

    ensure_dir(params.parent)
    params.write_text(
        json.dumps(
            {
                "schema": schema,
                "runs": [
                    {
                        "epsilon": _json_number(epsilon),
                        "delta": DPSYN_DELTA,
                        "max_records": int(record_count),
    ...
    datatype.write_text(json.dumps({"dtype": dtype_map}, indent=2) + "\n", encoding="utf-8")
    marginal.write_text(
        f"---\npriv_all_two_way:\n  total_eps: {_json_number(epsilon)}\n",
        encoding="utf-8",
    )
...
def _generate_proposed_task(task: tuple[str, str, float, int]) -> str:
    root, dataset_value, total_eps, idx = task
    ctx = _single_worker_context(root)
    ...
    return str(shared_path(ctx, dataset, total_eps, "proposed", idx=idx))
...
def maybe_generate_proposed(ctx: Context, datasets: list[DATASET], include_large_mia: bool, validate_only: bool, copies: int) -> None:
    ...
    total_slots = len(datasets) * len(eps_list) * copies
    pbar = tqdm(total=total_slots, desc="Generate proposed", dynamic_ncols=True)
        ...
        for eff_eps in eps_list:
            ...
            for idx in range(copies):
                ...
                if validate_only:
                    ...
                    continue
                if ctx.dry_run:
                    print(f"[dry-run] would generate proposed dataset {proposed_out}")
                    pbar.update(1)
                    continue
                if proposed_out.exists():
                    pbar.update(1)
                    continue
        ...
        for eff_eps in eps_list:
            ...
            for idx in range(copies):
                ...
                if validate_only:
                    ...
                    continue
                if ctx.dry_run:
                    print(f"[dry-run] would generate ldp dataset {ldp_out}")
                elif not ldp_out.exists():
        ...
        for eff_eps in STANDARD_EFFECTIVE_EPS:
            ...
            for idx in range(copies):
                ...
                if validate_only:
                    ...
                    continue
                if ctx.dry_run:
                    print(f"[dry-run] would generate proposed 100-SNP dataset {out}")
                elif not out.exists():
...
def run_privbayes_generation(ctx: Context, datasets: list[DATASET], validate_only: bool, copies: int) -> None:
    script = ctx.comparison_methods_dir / "PrivBayes" / "experiment.py"
    ...
    for dataset in datasets:
        for eff_eps in STANDARD_EFFECTIVE_EPS:
            ...
            for idx in range(copies):
                ...
                if baseline_output_is_valid(ctx, dataset, total_eps, "privbayes", snp_count=100, idx=idx):
                    pbar.update(1)
                    continue
                if validate_only:
                    pbar.update(1)
                    continue
                if ctx.dry_run:
                    print(f"[dry-run] would run PrivBayes for {dataset.value} eps={total_eps} idx={idx} -> {out}")
                    pbar.update(1)
                    continue
                if out.exists() and not ctx.dry_run:
                    out.unlink()
                record_count = load_target_data(ctx, dataset, snp_count=100, idx=idx).shape[0]
                cmd = [
                    "python",
                    str(script),
                    dataset.value,
                ...
                subprocess.run(cmd, cwd=ctx.comparison_methods_dir / "PrivBayes", check=True)
                if not baseline_output_is_valid(ctx, dataset, total_eps, "privbayes", snp_count=100, idx=idx):
                    raise RuntimeError(f"PrivBayes generation produced an invalid output for {dataset.value} eps={total_eps} idx={idx}")
                pbar.update(1)
    pbar.close()


def run_dpsyn_generation(ctx: Context, datasets: list[DATASET], validate_only: bool, copies: int) -> None:
    script = ctx.comparison_methods_dir / "DPSyn" / "experiment.py"
    if not script.exists():
    ...
    for dataset in datasets:
        for eff_eps in STANDARD_EFFECTIVE_EPS:
            ...
            for idx in range(copies):
                ...
                if baseline_output_is_valid(ctx, dataset, total_eps, "dpsyn", snp_count=100, idx=idx):
                    pbar.update(1)
                    continue
                if validate_only:
                    pbar.update(1)
                ...
                record_count = load_target_data(ctx, dataset, snp_count=100, idx=idx).shape[0]
                params = ctx.comparison_methods_dir / "DPSyn" / "config" / f"parameters_{dataset.value}_{total_eps:.4f}_{record_count}_{idx}.json"
                datatype = ctx.comparison_methods_dir / "DPSyn" / "config" / f"column_datatypes_{dataset.value}_{total_eps:.4f}_{record_count}_{idx}.json"
                marginal = ctx.comparison_methods_dir / "DPSyn" / "config" / f"eps_{total_eps:.4f}.yaml"
                target_path = out
                stale_pickle = (
                    ctx.comparison_methods_dir
                    / "DPSyn"
                    / "data"
                    / "pkl"
                    / f"preprocessed_priv_{dataset.value}_{total_eps:.4f}_{record_count}_{idx}.pkl"
                )

                if ctx.dry_run:
                    print(
                        f"[dry-run] would generate DPSyn config files {params.name}, "
                        f"{datatype.name}, {marginal.name}"
                    )
                    print(f"[dry-run] would run DPSyn for {dataset.value} eps={total_eps} idx={idx} -> {out}")
                    pbar.update(1)
                    continue

                write_dpsyn_run_config(
                    priv_data=priv_data,
                    params=params,
                    datatype=datatype,
                    marginal=marginal,
                ...
                ensure_dir(stale_pickle.parent)
                if stale_pickle.exists() and not ctx.dry_run:
                    stale_pickle.unlink()
                if out.exists() and not ctx.dry_run:
                    out.unlink()
                ...
                cmd = [
                    ...
                    str(priv_data),
                    "--priv_data_name",
                    f"{dataset.value}_{total_eps:.4f}_{record_count}_{idx}",
                    "--config",
                    str(ctx.comparison_methods_dir / "DPSyn" / "config" / "data.yaml"),
                    "--n",
                    str(record_count),
                    "--params",
                    str(params),
                ...
                subprocess.run(cmd, cwd=ctx.comparison_methods_dir / "DPSyn", check=True)
                if not baseline_output_is_valid(ctx, dataset, total_eps, "dpsyn", snp_count=100, idx=idx):
                    raise RuntimeError(f"DPSyn generation produced an invalid output for {dataset.value} eps={total_eps} idx={idx}")
                pbar.update(1)
    pbar.close()
...
def run_mia_experiment_with_split(target_data: np.ndarray, shared_data: np.ndarray, reference_data: np.ndarray, experiment_name: str) -> float:
    ...
    if experiment_name == "xgboost":
        ...
        model = xgb.XGBClassifier(eval_metric="logloss", n_jobs=1).fit(X_train, y_train)
        pred = model.predict(X_test)
    elif experiment_name == "decision_tree":
        model = DecisionTreeClassifier().fit(X_train, y_train)
        pred = model.predict(X_test)
    elif experiment_name == "random_forest":
        model = RandomForestClassifier(max_depth=2, n_jobs=1).fit(X_train, y_train)
        pred = model.predict(X_test)
...
def _single_worker_context(root: str | Path) -> Context:
    return Context(root=Path(root), workers=1, dry_run=False, no_overwrite_results=False)


def _evaluate_gwas_task(task: tuple[str, str, str, str, float, int]) -> list[dict]:
    root, dataset_value, gwas_type, error_type, method, eff_eps, idx = task
    ctx = _single_worker_context(root)
    dataset = DATASET(dataset_value)
    base = EPSILON_BASES[dataset]
...
def _evaluate_mia_task(task: tuple[str, str, str, str, float, int]) -> dict:
    root, dataset_value, approach, method_name, eff_eps, idx = task
    ctx = _single_worker_context(root)
    dataset = DATASET(dataset_value)
    base = EPSILON_BASES[dataset]
    target = load_target_data(ctx, dataset)
    reference = load_reference_data(ctx, dataset)
    shared = load_shared_data(ctx, dataset, eff_eps * base, approach, idx=idx)
...
def _evaluate_gwas_maf_task(task: tuple[str, str, str, str, float, int]) -> list[dict]:
    root, dataset_value, gwas_type, error_type, eps_maf, idx = task
    ctx = _single_worker_context(root)
    dataset = DATASET(dataset_value)
    original = load_target_data(ctx, dataset)
    reference = load_reference_data(ctx, dataset)
...
def _evaluate_utility_task(task: tuple[str, str, str, float, int]) -> dict:
    root, dataset_value, metric_name, method, eff_eps, idx = task
    ctx = _single_worker_context(root)
    dataset = DATASET(dataset_value)
    original = load_target_data(ctx, dataset)
    shared = load_shared_data(ctx, dataset, eff_eps * EPSILON_BASES[dataset], method, idx=idx)
    metric_fn = {
...
def _evaluate_utility_100_task(task: tuple[str, str, str, str, float, int]) -> dict:
    root, dataset_value, metric_name, method, eff_eps, idx = task
    ctx = _single_worker_context(root)
    dataset = DATASET(dataset_value)
    original = load_target_data(ctx, dataset, snp_count=100, idx=idx)
    shared = load_shared_data(ctx, dataset, 100 * eff_eps, method, snp_count=100, idx=idx)
    metric_fn = {
        "point_error": calc_point_error,
        "calc_sample_distance": calc_sample_distance,
        "mean_error": calc_mean_error,
...
def evaluate_gwas(ctx: Context, datasets: list[DATASET], copies: int) -> None:
    ...
    if tasks:
        if ctx.workers > 1 and not ctx.dry_run:
            chunksize = max(1, len(tasks) // max(1, ctx.workers * 8))
            with mp.Pool(processes=ctx.workers) as pool:
                for task_rows in pool.imap_unordered(_evaluate_gwas_task, tasks, chunksize=chunksize):
                    rows.extend(task_rows)
                    pbar.update(1)
        else:
            for task in tasks:
                rows.extend(_evaluate_gwas_task(task))
                pbar.update(1)

    ensure_dir(ctx.results_dir)
    df = clean_dataframe(pd.DataFrame(rows))
    out = write_results_dataframe(ctx, "gwas_df_full.csv", df)
    pbar.close()
    print(f"[done] {out}")
    if ctx.dry_run:
        print_dry_run_preview("GWAS standard", df)
...
def evaluate_gwas_maf(ctx: Context, datasets: list[DATASET], copies: int) -> None:
    ...
    if tasks:
        if ctx.workers > 1 and not ctx.dry_run:
            chunksize = max(1, len(tasks) // max(1, ctx.workers * 8))
            with mp.Pool(processes=ctx.workers) as pool:
                for task_rows in pool.imap_unordered(_evaluate_gwas_maf_task, tasks, chunksize=chunksize):
                    rows.extend(task_rows)
                    pbar.update(1)
        else:
            for task in tasks:
                rows.extend(_evaluate_gwas_maf_task(task))
                pbar.update(1)
    ...
    for dataset in datasets:
        ...
        for method_name in MIA_METHODS:
            ...
            for eff_eps in eps_list:
                total_eps = eff_eps * base
                for idx in range(copies):
                    for approach in ["proposed", "ldp"]:
                        if shared_path(ctx, dataset, total_eps, approach, idx=idx).exists():
                            tasks.append((str(ctx.root), dataset.value, approach, method_name, eff_eps, idx))

    pbar = tqdm(total=len(tasks), desc=f"MIA ({out_name})", dynamic_ncols=True)
    if tasks:
        if ctx.workers > 1 and not ctx.dry_run:
            chunksize = max(1, len(tasks) // max(1, ctx.workers * 8))
            with mp.Pool(processes=ctx.workers) as pool:
                for row in pool.imap_unordered(_evaluate_mia_task, tasks, chunksize=chunksize):
                    rows.append(row)
                    pbar.update(1)
        else:
            for task in tasks:
                rows.append(_evaluate_mia_task(task))
                pbar.update(1)

    df = clean_dataframe(pd.DataFrame(rows))
    out = write_results_dataframe(ctx, out_name, df)
    pbar.close()
    print(f"[done] {out}")
    if ctx.dry_run:
...
def evaluate_utility(ctx: Context, datasets: list[DATASET], copies: int) -> None:
    metrics = ["point_error", "calc_sample_distance", "mean_error", "variance_error"]
    rows: list[dict] = []
    methods = ["proposed", "ldp"]
    tasks: list[tuple[str, str, str, str, float, int]] = []
    for dataset in datasets:
        ...
        for metric_name in metrics:
            for method in methods:
                for eff_eps in STANDARD_EFFECTIVE_EPS:
                    total_eps = eff_eps * base
                    for idx in range(copies):
                        if shared_path(ctx, dataset, total_eps, method, idx=idx).exists():
                            tasks.append((str(ctx.root), dataset.value, metric_name, method, eff_eps, idx))

    pbar = tqdm(total=len(tasks), desc="Utility standard", dynamic_ncols=True)
    if tasks:
        if ctx.workers > 1 and not ctx.dry_run:
            chunksize = max(1, len(tasks) // max(1, ctx.workers * 8))
            with mp.Pool(processes=ctx.workers) as pool:
                for row in pool.imap_unordered(_evaluate_utility_task, tasks, chunksize=chunksize):
                    rows.append(row)
                    pbar.update(1)
        else:
            for task in tasks:
                rows.append(_evaluate_utility_task(task))
                pbar.update(1)
...
def evaluate_utility_100(ctx: Context, datasets: list[DATASET], copies: int) -> None:
    metrics = ["point_error", "calc_sample_distance", "mean_error", "variance_error"]
    rows: list[dict] = []
    methods = ["proposed", "privbayes", "dpsyn"]
    tasks: list[tuple[str, str, str, str, float, int]] = []
    for dataset in datasets:
        for metric_name in metrics:
            for method in methods:
                for eff_eps in STANDARD_EFFECTIVE_EPS:
                    total_eps = 100 * eff_eps
                    for idx in range(copies):
                        if shared_path(ctx, dataset, total_eps, method, snp_count=100, idx=idx).exists():
                            tasks.append((str(ctx.root), dataset.value, metric_name, method, eff_eps, idx))

    pbar = tqdm(total=len(tasks), desc="Utility 100-SNP", dynamic_ncols=True)
    if tasks:
        if ctx.workers > 1 and not ctx.dry_run:
            chunksize = max(1, len(tasks) // max(1, ctx.workers * 8))
            with mp.Pool(processes=ctx.workers) as pool:
                for row in pool.imap_unordered(_evaluate_utility_100_task, tasks, chunksize=chunksize):
                    rows.append(row)
                    pbar.update(1)
        else:
            for task in tasks:
                rows.append(_evaluate_utility_100_task(task))
                pbar.update(1)
...
def validate_inputs(ctx: Context, datasets: list[DATASET], include_large_mia: bool, only_100_snp: bool, copies: int) -> int:
    ...
    for p in [ctx.comparison_methods_dir / "PrivBayes" / "experiment.py", ctx.comparison_methods_dir / "DPSyn" / "experiment.py"]:
        if not p.exists():
            print(f"[warn] optional missing comparison runtime: {p}")
