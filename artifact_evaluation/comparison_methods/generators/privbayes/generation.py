"""Generation entrypoint for the 100-SNP PrivBayes baseline."""
...
def run(datasets: str = DEFAULT_DATASETS, copies: int = DEFAULT_COPIES, dry_run: bool = False) -> int:
    ...
    if code != 0:
        return code
    run_privbayes_generation(ctx, selected, validate_only=False, copies=copies)
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate 100-SNP datasets for PrivBayes.")
