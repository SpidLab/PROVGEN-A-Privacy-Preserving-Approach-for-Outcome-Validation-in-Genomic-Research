"""Run the standard-scale MIA evaluation."""
...
def run(
    ...
    code = validate_evaluation(ctx, selected, include_large_mia=False, only_100_snp=False, copies=copies)
    if code != 0:
        return code
    if dry_run:
        report_evaluation_dry_run(ctx, "mia_experiments_results_full.csv", selected, copies)
        return 0
    evaluate_mia(ctx, selected, STANDARD_EFFECTIVE_EPS, "mia_experiments_results_full.csv", copies=copies)
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the standard-scale MIA evaluation.")
