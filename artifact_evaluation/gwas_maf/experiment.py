"""Run the GWAS MAF evaluation from the paper."""
...
def run(
    ...
    if code != 0:
        return code
    if dry_run:
        report_evaluation_dry_run(ctx, "gwas_df_full_maf.csv", selected, copies)
        return 0
    evaluate_gwas_maf(ctx, selected, copies=copies)
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the GWAS MAF evaluation.")
